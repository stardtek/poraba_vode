...
class Model_zgodovina extends CI_Model {
    ...
    public function usee()
	{
        ...
        //$this->db->join('mesec', 'mesec.idvoda = voda.idvoda');
        #$this->db->join('vrstavode', 'vrstavode.idvoda=voda.idvoda');
        
      
        #$query=$this->db->query('SELECT * FROM voda where iduporabnika like "'.$_SESSION['uporabnik'].'"');
        #$vrstice=$query->num_rows();
        #echo $query->num_rows();
        #$st= $query->row('kolicina');
        #echo $st;
        #print_r($query);
        #'SELECT * FROM voda join vrstavode on vrstavode.idvoda = voda.idvoda join mesec on mesec.idvoda = voda.idvoda where iduporabnika = '
